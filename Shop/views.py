...
def feedback(request):
    if 'shid' in request.session:
        if request.method == "POST":
            db.collection("tbl_feedback").add({"feedback_content":request.POST.get("txt_feedback"),"shop_id":request.session["shid"]})
            return render(request,"Shop/FeedBack.html",{"msg":"FeedBack Sended.."})
        else:
